...
def mail_fetch(service=None, token_data=None, start_date=None, max_results=10, query='in:inbox'):
    """
    Fetch emails from Gmail (replaces EZGmail functionality)
    ...
    try:
        ...
        for i, msg in enumerate(messages):
            try:
                ...
                # Extract email data in format similar to EZGmail
                email_data = {
                    "email_number": i + 1,
                    "id": message['id'],
...
def send_email(service, to, subject, body, cc=None, bcc=None):
    ...
    Args:
        ...
        cc: CC recipients (list or string)
        bcc: BCC recipients (list or string)
    
    Returns:
        Sent message dict or None if error
    """
    try:
...
def save_emails_to_json(emails, filename='emails.json'):
    ...
    Args:
        ...
        filename: Output filename (default: 'emails.json')
...
if __name__ == "__main__":
    try:
        ...
        if emails:
            ...
            # Save to JSON file
            save_emails_to_json(emails, 'emails_output.json')
        ...
        if unread:
            ...
            save_emails_to_json(unread, 'unread_emails.json')
        else:
